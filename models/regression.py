...
class Regression():
    def __init__(self, n_feats_to_select, reg_model, get_summary=True, 
        ...
        if self.reg_model == 'binary_logit':
            self.model = LogisticRegression(max_iter=2000, class_weight='balanced')
            self.estimator = LogisticRegression(max_iter=2000, class_weight='balanced')
    ...
    def _select_features(self, x:pd.DataFrame, y:pd.Series):
        ...
        print(f'Features are selected based on RFE: {selected_feats}')
        return selected_feats
    
    def _get_summartry(self, train_x:pd.DataFrame, train_y:pd.Series):

        self.selected_feats = self._select_features(train_x, train_y)
        self.os_train_x = train_x[self.selected_feats]

        if self.reg_model == 'binary_logit':
            model = sm.Logit(train_y, self.os_train_x)
        result = model.fit()
    ...
    def train(self, train_x:pd.DataFrame, train_y:pd.Series):
        ...
        if self.get_summary:
            summary = self._get_summartry(train_x, train_y)
            print(summary)
    ...
    def test(self, test_x:pd.DataFrame, test_y:pd.Series):
        ...
        print(f'Accuracy of logistic regression classifier on test set: {accuracy:.4f}')
        ...
        # Compute comprehensive metrics (same as ResNet/SVM/RF)
    ...
    def _compute_all_metrics(self, test_y, pred_y, pred_y_prob, num_classes):
        """Compute all metrics similar to ResNet/SVM/RF evaluation."""
        print(f'\n{"="*60}')
    ...
    def _plot_roc(self, test_y, pred_y_prob, num_classes):
        ...
        plt.grid(alpha=0.3)
        plt.savefig('Log_ROC.png', dpi=300, bbox_inches='tight')
        print('\nROC curve saved as Log_ROC.png')
        plt.close()
