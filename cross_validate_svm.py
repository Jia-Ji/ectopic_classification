"""
Cross-validation script for SVM model across multiple datasets.
...
def train_and_test_svm(cfg: DictConfig, dataset_name: str, run_idx: int, total_datasets: int) -> dict:
    """Train and test SVM model for a single dataset, return metrics."""
    print(f"\n{'='*80}")
    ...
    # Train the model
    print(f"\nTraining SVM on dataset: {dataset_name}...", flush=True)
    model.train(train_features, train_labels)
    
    # Test the model
    print(f"\nTesting SVM on dataset: {dataset_name}...", flush=True)
...
def main():
    ...
    print("="*80)
    print("SVM CROSS-VALIDATION ACROSS MULTIPLE DATASETS")
    print("="*80)
    
    # Load base config to get base_path
    config_path = "config"
    with initialize(config_path=config_path, version_base=None):
        cfg = compose(config_name="train_svm_config")
    ...
    for idx, dataset_name in enumerate(datasets):
        try:
            metrics = train_and_test_svm(cfg, dataset_name, idx, len(datasets))
            if metrics:
    ...
    individual_path = os.path.join(results_dir, "svm_individual_metrics.csv")
    individual_df.to_csv(individual_path)
    print(f"\nSaved individual metrics to: {individual_path}")
    
    # Save statistics
    stats_df = pd.DataFrame([stats])
    stats_path = os.path.join(results_dir, "svm_mean_std_metrics.csv")
    ...
    print("\n" + "="*80)
    print("SVM Cross-validation completed!")
