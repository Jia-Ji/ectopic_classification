...
def get_ppgSQI(ppg: list, fs: int, annotation: list):
    ...
    # Convert annotation to numpy array if needed and handle empty case
    annotation = np.asarray(annotation)
    if len(annotation) < 2:
        return np.zeros(max(0, len(annotation) - 1))
    ...
    if t is None or v == 0:
        # Return array of zeros when template is invalid
        return np.zeros(len(annotation) - 1)
