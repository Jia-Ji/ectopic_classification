"""
Cross-validation script for Regression model across multiple datasets.
...
def train_and_test_regression(cfg: DictConfig, dataset_name: str, run_idx: int, total_datasets: int) -> dict:
    """Train and test Regression model for a single dataset, return metrics."""
    print(f"\n{'='*80}")
    ...
    # Train the model
    print(f"\nTraining Regression on dataset: {dataset_name}...", flush=True)
    model.train(train_features, train_labels)
    
    # Test the model
    print(f"\nTesting Regression on dataset: {dataset_name}...", flush=True)
...
def main():
    ...
    print("="*80)
    print("REGRESSION CROSS-VALIDATION ACROSS MULTIPLE DATASETS")
    print("="*80)
    
    # Load base config to get base_path
    config_path = "config"
    with initialize(config_path=config_path, version_base=None):
        cfg = compose(config_name="train_reg_config")
    ...
    for idx, dataset_name in enumerate(datasets):
        try:
            metrics = train_and_test_regression(cfg, dataset_name, idx, len(datasets))
            if metrics:
    ...
    individual_path = os.path.join(results_dir, "regression_individual_metrics.csv")
    individual_df.to_csv(individual_path)
    print(f"\nSaved individual metrics to: {individual_path}")
    
    # Save statistics
    stats_df = pd.DataFrame([stats])
    stats_path = os.path.join(results_dir, "regression_mean_std_metrics.csv")
